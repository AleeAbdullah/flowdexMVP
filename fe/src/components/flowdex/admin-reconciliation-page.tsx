      ...
      <GlassPanel className="overflow-hidden">
        ...
        <div>
          {unmatchedQuery.isLoading ? (
            <div className="px-6 py-5 text-sm text-[var(--flowdex-muted)]">Loading unmatched events...</div>
          ) : null}
          ...
          {items.map(item => (
            <div
              key={item.id}
              className="grid gap-4 border-b border-[var(--flowdex-card-border)] px-6 py-5 last:border-b-0 xl:grid-cols-[1.2fr_1fr_1fr]"
            >
              <div className="space-y-2">
                <div className="font-semibold text-[var(--flowdex-text)]">{item.assetCode} on {item.chain}</div>
                <div className="text-sm text-[color-mix(in_srgb,var(--flowdex-text)_45%,transparent)]">Tx {truncateMiddle(item.txHash)}</div>
                <div className="text-sm text-rose-200">{item.reconciliationReason ?? 'No reason attached'}</div>
              </div>
              <div className="space-y-2 text-sm text-[var(--flowdex-muted)]">
