      ...
      <div className="grid gap-4 xl:grid-cols-3">
        {props.highlights.map(item => (
          <GlassPanel key={item.title} className="p-5">
            <div className="text-[10px] font-bold tracking-[0.28em] text-slate-500 uppercase">{item.title}</div>
            <div className="font-data mt-4 text-2xl text-white">{item.value}</div>
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="mt-1 text-sm text-slate-400">{item.status}</div>
                </div>
              </div>
