      ...
      <div className="grid gap-4 xl:grid-cols-3">
        {props.highlights.map(item => (
          <GlassPanel key={item.title} className="p-5">
            <div className="text-[10px] font-bold tracking-[0.28em] text-[color-mix(in_srgb,var(--text)_52%,transparent)] uppercase">{item.title}</div>
            <div className="font-data mt-4 text-2xl text-[var(--text)]">{item.value}</div>
