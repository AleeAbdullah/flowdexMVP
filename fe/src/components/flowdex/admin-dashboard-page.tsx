  ...
  if (statsQuery.isError || !stats) {
    return (
      <GlassPanel className="border border-rose-400/20 bg-rose-500/10 p-6">
        <div className="text-lg font-bold text-white">Admin summary unavailable</div>
    ...
    <div className="space-y-8">
      ...
      <div className="grid gap-4 xl:grid-cols-4">
        <GlassPanel className="p-5">
          ...
          <p className="mt-3 text-sm leading-7 text-slate-300">Confirmed presale volume from finalized on-chain lifecycle state.</p>
        </GlassPanel>
        <GlassPanel className="p-5">
          <DataKicker label="Confirmed Volume (Display)" value={formatCurrency(stats.totalConfirmedVolumeDisplay, 0)} />
          <p className="mt-3 text-sm leading-7 text-slate-300">Display-layer marketing multiplier applied on the backend response.</p>
        </GlassPanel>
        <GlassPanel className="p-5">
          <DataKicker label="Unmatched Events" value={`${stats.unmatchedCount}`} />
          <p className="mt-3 text-sm leading-7 text-slate-300">Chain events that still need operational review or reconciliation context.</p>
        </GlassPanel>
        <GlassPanel className="p-5">
          <DataKicker label="Refund Queue" value={`${stats.refundCount}`} />
          <p className="mt-3 text-sm leading-7 text-slate-300">Refund records currently tracked by the backend admin lifecycle.</p>
        </GlassPanel>
      </div>
      ...
      <GlassPanel className="p-6">
        <div className="text-[10px] font-bold tracking-[0.28em] text-slate-500 uppercase">Lifecycle Status Mix</div>
        <div className="mt-5 grid gap-4 md:grid-cols-3 xl:grid-cols-6">
          {Object.entries(stats.transactionCountsByStatus).map(([status, count]) => (
            <div key={status} className="rounded-[1.2rem] border border-white/8 bg-white/4 p-4">
              <div className="text-[10px] font-semibold tracking-[0.28em] text-slate-500 uppercase">{status}</div>
              <div className="font-data mt-3 text-2xl text-white">{count}</div>
            </div>
          ))}
        </div>
      </GlassPanel>
