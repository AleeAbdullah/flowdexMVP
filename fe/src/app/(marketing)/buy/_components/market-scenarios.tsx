  ...
  return (
    <section>
      <div className="text-[10px] font-bold tracking-[0.32em] text-[var(--cyan)] uppercase">
        <span className="bg-[var(--cyan)] px-1.5 py-1 text-[#02111c]">Market</span>
      ...
      <div className="mt-7 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        {props.cards.map(card => (
          <GlassPanel key={card.label} className="rounded-[0.75rem] bg-[#050c16] px-5 py-7 text-center">
            <div className="text-xl font-black text-[var(--cyan)]">{card.label}</div>
            <div className="mt-5 space-y-1 text-[10px] text-[var(--muted)]">
              <div>{card.price}</div>
              <div>{card.cap}</div>
            </div>
            <div className="mt-7 text-2xl font-black text-emerald-300">{card.value}</div>
            <div className="mt-2 text-xs font-bold text-emerald-300">{card.roi}</div>
          </GlassPanel>
        ))}
      </div>
    </section>
